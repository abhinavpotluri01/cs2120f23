...
def not_either_not_both { jam cheese } :
  ((no jam) ⊕ (no cheese)) → 
  (no (jam × cheese)) 
| Sum.inl nojam => λ(jc : jam × cheese) => nojam jc.1
| Sum.inr nocheese => λ(jc : jam × cheese) => nocheese jc.2
...
| (Sum.inl noa) => λ(ab : α × β) => noa ab.1
| (Sum.inr nob) => λ(ab : α × β) => nob ab.2
...
#reduce sum_f square 5 --expect 55
#reduce sum_f square 0 --expect 0
#reduce sum_f square 1 --excpect 1
